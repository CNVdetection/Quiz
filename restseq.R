#This code is used to extract information, from restseq data

mydata = read.table("D:/restseq_data_hg_sorted.hits")
...
for (i in 1:L){
     ...
     if ((i<=L)&&(chromo==mydata[i+1,2])&&(mapping_position==mydata[i+1,3])){
         number_of_indels <- number_of_indels + (indels[i+1,2]+indels[i+1,3]);
         if ((indels[i+1,2]+indels[i+1,3])>0){
              rwi <- rwi + 1;
         }
         number_of_read <- number_of_read +1;
            ...
            if (s==1){
             ...
             number_of_indels <- indels[i+1,2]+indels[i+1,3]
             if ((indels[i+1,2]+indels[i+1,3])>0){
                  rwi <- 1
             }else{
                  rwi <- 0
...
write.table(ALL,"D:\\all2.txt",sep = "\t",col.names = FALSE,row.names = FALSE)